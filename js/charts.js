...
const Charts = {
  renderDashboardSpline() {
    ...
    ctx.fillStyle = '#756568'; // Muted text
    ...
    for (let i = 0; i < 5; i++) {
      ...
      ctx.strokeStyle = 'rgba(117, 101, 104, 0.15)';
      ctx.lineWidth = 1;
      ctx.beginPath();
    ...
    areaGrad.addColorStop(0, 'rgba(180, 35, 60, 0.28)');
    areaGrad.addColorStop(1, 'rgba(180, 35, 60, 0.01)');
    ...
    points.forEach((p, idx) => {
      ...
      if (idx === 5) {
        ...
        ctx.strokeStyle = 'rgba(180, 35, 60, 0.4)';
        ctx.lineWidth = 1.5;
        ctx.stroke();
      }
    });
    ...
    ctx.strokeStyle = 'rgba(116, 21, 42, 0.55)';
    ctx.lineWidth = 1.8;
    ctx.setLineDash([4, 4]);
    ctx.stroke();
    ctx.setLineDash([]);
  ...
  renderEventDistributionDonut() {
    ...
    const segments = [
      { label: 'Firewall', pct: 0.25, color: '#B4233C' },
      { label: 'OKTA Auth', pct: 0.31, color: '#74152A' },
      { label: 'Suspicious', pct: 0.24, color: '#C47A16' },
      { label: 'Endpoints', pct: 0.13, color: '#756568' },
      { label: 'Cloud Audit', pct: 0.07, color: '#E8A0AA' }
    ];
